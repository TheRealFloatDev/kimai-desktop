...
export const useStartTimer = () => {
  ...
  return useMutation({
    ...
    onMutate: () => {
      void kimaiApi.resetTimerDisplayAnchor();
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["liveDuration"] });
    },
    onError: () => {
      void kimaiApi.clearTimerDisplayAnchor();
    },
  });
};
...
export function useLiveDuration(timer: TimesheetCollectionExpanded | null | undefined) {
  return useQuery({
    queryKey: ["liveDuration", timer?.id],
    queryFn: async () => {
      if (!timer) return 0;
      const secs = await kimaiApi.getTimerDisplaySeconds();
      return secs ?? 0;
    },
    enabled: !!timer,
